...
$(".add-table-items").on('click','.remove-btn',function(event){
    event.preventDefault(); // Prevent the default behavior of the event
    $(this).closest('.add-row').remove(); // Remove the closest element with class "add-row"
});

$(document).ready(function() {
    // Counter for number of rows added
    var rowCount = 1;
  
    $(document).on("click", ".add-btn", function(event) {
      event.preventDefault(); // Prevent the default behavior of the event
  
      // Check if maximum number of rows have been added
      if (rowCount >= 10) {
        alert("Maximum limit of 10 rows reached");
        return false;
      }
    ...
    $(document).on("click", ".remove-btn", function(event) {
      ...
      // Check if parent row is being deleted
      if ($(this).closest('tr').hasClass('parent-row')) {
